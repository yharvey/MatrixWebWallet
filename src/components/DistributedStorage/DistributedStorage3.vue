<template>
  <div class="ai-distributedStorage">
    <div class="distributedStorage3">
      <div class="sizeStyle">
        {{$t('Please select the mint NFT  data file to upload')}}
      </div>
...
export default {
  data () {
    return {
      ...
      uploadProgress: 0,
      address: '',
      MANIAAddress: 'MAN.2gRxHsKucf5EeDNi9Bg87EGG7Zxft',
      getGas: 0,
      ruleForm: {
        value: 0,
        to: '',
  ...
  methods: {
    ...
    changeFile () {
      ...
      this.ipfsProvider.add(fileUpload, {
        ...
        this.ruleForm.data = contract.createNFT.getData(this.dataHash)
        this.estimate()
      })
    },
    upload () {
      ...
      try {
        this.$refs['ruleForm2'].validate((valid) => {
          if (valid) {
            ...
            this.ruleForm.to = this.MANIAAddress
            ...
            let contract = this.httpProvider.man
              .contract(abiArray)
              .at(contractAddress)
            jsonObj.data = contract.createNFT.getData(this.dataHash)
